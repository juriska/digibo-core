CREATE OR REPLACE package body IB.BOPayment as
...
begin
    ...
    if (statuses = '20' or statuses = '21' or statuses = '20,21' ) and
       (pmtClass = '25' or pmtClass = '26' or instr( pmtClass, '25', 1) > 0 or instr( pmtClass, '26', 1) > 0 ) then
       pmtClass := pmtClass || ',509';
    end if;
...
begin
	...
	select /* BOPayment.payment */
		...
		decode(d.creator_channel_id,
            ...
            RBA_CONST.QUICK_AUTH, 'IBTT' || d.id,
        ...
        c.sector,
        c.segment,
        decode( d.class_id, 134, D.PHONE_MOBILE, 135, D.PHONE_MOBILE, '')
	into
		userName,
		userId,
		officerName,
...
begin
    null;
    select count(1) into cnt from BANK_TEMPLATE_CONFIG where document_id = pId;
    if cnt > 1 then
       update BANK_TEMPLATE_CONFIG set group_id = pGroup where document_id = pId;
    else
       insert into  BANK_TEMPLATE_CONFIG ( document_id, group_id) values (pId, pGroup);
    end if;
end;
