/*
* Account administration orders.
*/

create or replace package BOaccadmin as
...
create or replace package body BOaccadmin as
...
begin
	...
	where p.id = docId
		and p.class_id in ( 507, 508, 717, 718, 791, 792)
...
function execute_by_filter(
	...
	rowset accadmin_set_t := accadmin_set_t();
	pId number(14);
	status number(2);
	created date;
...
begin
	...
	dbms_sql.define_column(cursor_name,  8, itc, 2000); -- 2015-03-20 manits 32 uz 2000
	dbms_sql.define_column(cursor_name,  9, class_id);
	dbms_sql.define_column(cursor_name,  10, country, 2);
	...
	while dbms_sql.fetch_rows(cursor_name) > 0 loop
		...
		dbms_sql.column_value(cursor_name,  10, country);
		row := accadmin_t(
			pId,
			status,
			created,
	...
	open rv for select * from table(cast(rowset as accadmin_set_t));
	return rv;
...
*/
	...
	rq := rq || 'select /* BOaccadmin.find_by_filter */';
...
begin
	...
	where rownum <= bocommon.ResultSetSize and
		p.class_id in ( 507, 508, 717, 718, 791, 792) and (
			(pOfficerId = 0 and p.change_officer_id is null and p.status_id in ( RBA_CONST.SIGNATURE_OK, RBA_CONST.CONFIRM_OK)
