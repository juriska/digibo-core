...
create or replace package BOLeaseWEB as
...
create or replace package body BOLeaseWEB as
...
begin
	open rv for select
        ...
        --null itc,
		p.class_id class_id
        , trim( (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in ('amexNameF', 'amexNameJ', 'winName1', 'cnkName1', 'firstName' ) and rownum <= 1 ) || ' ' || (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in ('winName2', 'cnkName2', 'lastName' ) and rownum <= 1 ) ) customerName
        , (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in ('amexPersCode', 'amexRegNum', 'winPersCode', 'cnkPersCode', 'persCode', 'personalCode') and rownum <= 1 ) legalId
	from documents p, ways_of_connection w
	where p.id = docId
		and p.class_id in ( 1014)
...
function execute_by_filter(
	...
	rowset leaseweb_set_t := leaseweb_set_t();
	pId number(14);
	status number(2);
	created date;
...
begin
	...
	open rv for select * from table(cast(rowset as leaseweb_set_t));
	return rv;
...
begin
	...
	rq := rq || 'select /* BOleaseweb.find_by_filter */';
...
--	end if;
    ...
    rq := rq || ' , (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in (''amexPersCode'', ''amexRegNum'', ''winPersCode'', ''cnkPersCode'', ''persCode'', ''personalCode'') and rownum <= 1 ) legalId';
