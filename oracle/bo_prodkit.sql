...
create or replace package BOprodkit as
...
create or replace package body BOprodkit as
...
begin
	...
	where p.id = docId
		and p.class_id in ( 712, 713, 714, 715, 716)
...
function execute_by_filter(
	...
	rowset prodkit_set_t := prodkit_set_t();
	pId number(14);
	status number(2);
	created date;
...
begin
	...
	open rv for select * from table(cast(rowset as prodkit_set_t));
	return rv;
...
begin
	...
	rq := rq || 'select /* BOprodkit.find_by_filter */';
...
begin
	...
	where rownum <= bocommon.ResultSetSize and
		p.class_id in ( 712, 713, 714, 715, 716) and (
