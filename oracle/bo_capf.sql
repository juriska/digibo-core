...
create or replace package BOcapf as
...
create or replace package body BOcapf as
...
begin
	open rv for select
        ...
        null itc,
		p.class_id class_id
        , trim( (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in ('clientName1' ) and rownum <= 1 ) || ' ' || (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in ('clientName2') and rownum <= 1 ) ) customerName
        , null legalId
	from documents p, ways_of_connection w
	where p.id = docId
		and p.class_id in ( 18)
...
begin
	...
	open rv for select * from table(cast(rowset as capf_set_t));
	return rv;
...
begin
	...
	rq := rq || 'select /* BOcapf.find_by_filter */';
...
--	end if;
    ...
    rq := rq || ' , trim( (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in (''clientName1'' ) and rownum <= 1 ) || '' '' || (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in (''clientName2'' ) and rownum <= 1 ) ) customerName';
    rq := rq || ' , (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in (''amexPersCode'', ''amexRegNum'', ''winPersCode'', ''cnkPersCode'') and rownum <= 1 ) legalId';
	...
	--rq := rq || ' and p.class_id in ( 1002, 1003, 1004, 1005)';
	rq := rq || ' and p.class_id in (' || docClass || ')';
	rq := rq || ' and p.status_id in (' || statuses || ')';
    if pCustomerName is not null then
       null;
       rq := rq || ' and trim( (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in (''clientName1'' ) and rownum <= 1 ) || '' '' || (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in (''clientName2'' ) and rownum <= 1 ) ) like :pCustomerName';
    end if;
    if pLegalId is not null then
       null;
       rq := rq || ' and (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in (''amexPersCode'', ''amexRegNum'', ''winPersCode'', ''cnkPersCode'') and rownum <= 1 ) like :pLegalId';
    end if;
...
begin
	...
	open rv for select
		...
		p.class_id class_id
        , trim( (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in ('clientName1' ) and rownum <= 1 ) || ' ' || (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in ('clientName2') and rownum <= 1 ) ) customerName
        --, trim( (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in ('clientName1' ) and rownum <= 1 ) ) customerName
        , (select e.additional_info from document_extensions e where e.document_id = p.id and dictionary_id in ('amexPersCode', 'amexRegNum', 'winPersCode', 'cnkPersCode') and rownum <= 1 ) legalId
	from documents p, ways_of_connection w
	where rownum <= bocommon.ResultSetSize and
		p.class_id in ( 1008) and (
			(pOfficerId = 0 and p.change_officer_id is null and p.status_id in ( RBA_CONST.CONFIRM_OK)
			) -- 13
			or
