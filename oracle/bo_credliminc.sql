/*
* Credit limit increase orders.
*/

create or replace package BOcredliminc as
...
create or replace package body BOcredliminc as
...
function execute_by_filter(
	...
	rowset credliminc_set_t := credliminc_set_t();
	pId number(14);
	status number(2);
	created date;
...
begin
    ...
    dbms_sql.define_column(cursor_name,  8, itc, 2000); -- mainits 2015-02-12
	...
	open rv for select * from table(cast(rowset as credliminc_set_t));
	return rv;
...
/*
    ...
    */
	...
	rq := rq || 'select /* BOcredliminc.find_by_filter */';
...
begin
	--select / * BOcredliminc.credliminc * /
		d.bank_reference,
		u.name || ' (' || w.login || ')',
		u.personal_id,
		remote_officer.name,
