/*
* Leasing Applications orders.
*/

create or replace package BOLeaseApplications as
...
create or replace package body BOLeaseApplications as
...
function execute_by_filter(
	...
	rowset custody_set_t := custody_set_t();
	pId number(14);
	status number(2);
	created date;
...
begin
	...
	open rv for select * from table(cast(rowset as custody_set_t));
	return rv;
...
begin
	...
	where rownum <= bocommon.ResultSetSize and
		p.class_id in ( 728) and (
			(pOfficerId = 0 /*and p.change_officer_id is null*/ and p.status_id in ( RBA_CONST.SIGNATURE_OK, RBA_CONST.CONFIRM_OK)) -- 5, 13 -- 2016-09-19 REMOVED and p.change_officer_id is null
			or
